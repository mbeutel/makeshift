...
{
    using MM1 = override_params_1_<NewParamsT, OldParamsT>;
    using all_parameters = type_sequence2<std::decay_t<decltype(select_parameter(MM1::template select<CategoriesT>(std::declval<const NewParamsT&>(), std::declval<const OldParamsT&>()), CategoriesT{ }))>...>;
};
    ...
    struct MAKESHIFT_EMPTY_BASES metadata_of_0<T, true> : override_params<declared_metadata_t<T>, default_metadata<T>>, metadata_tag
{
    constexpr metadata_of_0(void)
        : override_params<declared_metadata_t<T>, default_metadata<T>>(reflect(type<T>{ }), { })
    {
    }
};
template <typename T>
