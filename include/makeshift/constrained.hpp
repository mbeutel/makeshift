...
template <typename T, T... Vs>
    std::string get_constrained_integer_hint(sequence<T, Vs...>, const enum_hint_options& options)
{
...
template <typename T, T... Vs>
    std::string get_constrained_integer_hint(std::integer_sequence<T, Vs...>, const enum_hint_options& options)
{
    return get_constrained_integer_hint(sequence<T, Vs...>{ }, options);
}
template <typename T, T First, T Last>
    std::string get_constrained_integer_hint(integer_range<T, First, Last>, const enum_hint_options& options)
{
...
template <typename T, T First, T Last>
    std::string get_constrained_integer_hint(integer_inclusive_range<T, First, Last>, const enum_hint_options& options)
{
    common_int<T> values[] = { common_int<T>(First), common_int<T>(Last) };
    return constrained_integer_hint(ConstraintType::inclusiveRange, values, 2, options);
}
template <typename T, T First>
    std::string get_constrained_integer_hint(integer_upper_half_range<T, First>, const enum_hint_options& options)
{
    common_int<T> values[] = { common_int<T>(First) };
    return constrained_integer_hint(ConstraintType::upperHalfRange, values, 1, options);
}
template <typename T, T Last>
    std::string get_constrained_integer_hint(integer_lower_half_range<T, Last>, const enum_hint_options& options)
{
    common_int<T> values[] = { common_int<T>(Last) };
    return constrained_integer_hint(ConstraintType::lowerHalfRange, values, 1, options);
}
template <typename T, T Last>
    std::string get_constrained_integer_hint(integer_lower_half_inclusive_range<T, Last>, const enum_hint_options& options)
{
    common_int<T> values[] = { common_int<T>(Last) };
...
struct default_integer_constraint_verifier
{
    ...
    template <typename ConstraintT>
        static std::string get_hint(const enum_hint_options& options, ConstraintT = { }) noexcept
    {
    ...
    struct default_hash<ConstrainedIntT, std::enable_if_t<is_constrained_integer_v<ConstrainedIntT>>>
{
    ...
    {
        return std::hash<typename ConstrainedIntT::value_type>{ }(arg.value());
    }
};
